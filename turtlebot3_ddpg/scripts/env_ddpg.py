...
class Env():
    ...
    def testing_dynamic(self): 
        ...
        dynamic_info = [['dynamic_cylinder_0', [-3.5, 1.57], 'vertical'], \
                        ['dynamic_cylinder_1', [-2.5, -0.53], 'vertical'], \
                        ['dynamic_cylinder_2', [-1.5, 0.72], 'vertical'], \
                        ['dynamic_cylinder_3', [-0.5, 1.26], 'vertical'], \
                        ['dynamic_cylinder_4', [0.5, 1.58], 'vertical'], \
                        ['dynamic_cylinder_5', [1.5, -1.06], 'vertical'], \
                        ['dynamic_cylinder_6', [2.5, 1.51], 'vertical'], \
                        ['dynamic_cylinder_7', [3.5, 1.92], 'vertical'], \
                        ['dynamic_cylinder_8', [-3.24, -1.5], 'horizontal'], \
                        ['dynamic_cylinder_9', [-1.17, -0.5], 'horizontal'], \
                        ['dynamic_cylinder_10', [3.40, 0.5], 'horizontal'], \
                        ['dynamic_cylinder_11', [-1.69, 1.5], 'horizontal']]
        for [model_name, position, moving_type] in dynamic_info:  # Add moving objects thread to thread list 
            locals()['moving_'+model_name] = MovingTesting(model_name, current_position=position, moving_type=moving_type) 
            thread_list.append(threading.Thread(name=model_name,target=locals()['moving_'+model_name].moving_obstacle))
        for i in range(len(thread_list)):  # Start all threads 
            thread_list[i].start()
